...
import { getSellerBids } from '../../../redux-store/slices/user'
...
const SellerAuctionBidsDialog = ({ open, onClose, bids = [], user = null }) => {
  const dispatch = useDispatch()

  const { sellerBids: userBids, loading } = useSelector(state => state.users)

  useEffect(() => {
    if (open && user) {
      dispatch(getSellerBids(user?._id))
    }
  }, [dispatch, user, open])
                  ...
                  <div className='flex-1 space-y-4'>
                    ...
                    <div className='flex items-center gap-3 p-3 rounded-lg bg-muted/50'>
                      <div className='w-10 h-10 rounded-full overflow-hidden border-2 border-primary/20'>
                        <img
                          src={getFullImageUrl(bid.user.profileImage)}
                          alt={bid.user.name}
                          className='w-full h-full object-cover'
                          onError={e => {
                            e.currentTarget.src = '/placeholder.svg'
...
export default SellerAuctionBidsDialog
